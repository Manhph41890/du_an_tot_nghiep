...
class OrderController extends Controller
{
    public function add(Request $request)
    {
        ...
        if ($paymentMethod->kieu_thanh_toan === 'Thanh toán bằng Vnpay') {
            ...
            if ($user->vi_nguoi_dungs->tong_tien < $totall) {
                return redirect()
                    ->back()
                    ->withErrors(['message' => 'Tiền trong tài khoản không đủ để thanh toán.']);
            }
            $order = new don_hang();
    ...
    public function applyCoupon(Request $request)
    {
        ...
        // 4. Kiểm tra giá trị mã khuyến mãi không lớn hơn tổng tiền
        if ($coupon->gia_tri_khuyen_mai > $totalAmount) {
            return response()->json(['success' => false, 'message' => 'Giá trị mã khuyến mãi không phù hợp với tổng tiền sản phẩm!']);
        }

        // 5. Tính toán số tiền giảm giá
        $discountAmount = $coupon->gia_tri_khuyen_mai; // Giảm tối đa đến tổng tiền
